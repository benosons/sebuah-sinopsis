...
<form id="category-form" method="POST" action="{{ route('admin.categories.update', $category) }}">
    ...
    <div class="max-w-2xl mx-auto flex flex-col gap-6">
        ...
        <div class="flex flex-col gap-1">
            <h2 class="text-3xl font-black tracking-tight text-gray-900 dark:text-white">Edit Kategori</h2>
            <p class="text-gray-500 dark:text-gray-400">Perbarui kategori "{{ $category->name }}"</p>
        </div>
            ...
            <div class="flex flex-col gap-5">
                ...
                <div class="flex flex-col md:flex-row gap-5">
                    <label class="flex flex-col flex-1">
                        ...
                        <input type="text" name="icon" value="{{ old('icon', $category->icon) }}" class="form-input w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white h-12 px-4 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors" placeholder="book, auto_stories, psychology"/>
                    ...
                    <label class="flex flex-col flex-1">
                        ...
                        <div class="flex gap-2">
                            <input type="color" name="color" value="{{ old('color', $category->color ?? '#0d59f2') }}" class="w-12 h-12 rounded-lg border border-gray-300 dark:border-gray-700 cursor-pointer"/>
                            <input type="text" id="color-text" value="{{ old('color', $category->color ?? '#0d59f2') }}" class="form-input flex-1 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white h-12 px-4 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors" placeholder="#0d59f2" readonly/>
                        </div>
                    </label>
                </div>
                ...
                <div class="pt-4 border-t border-gray-100 dark:border-gray-700">
                    ...
                    <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-100 dark:bg-gray-800">
                        <span id="preview-icon" class="material-symbols-outlined text-lg" style="color: {{ $category->color ?? '#0d59f2' }}">{{ $category->icon ?? 'category' }}</span>
                        <span id="preview-name" class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $category->name }}</span>
                    </div>
                </div>
            </div>
        </div>
