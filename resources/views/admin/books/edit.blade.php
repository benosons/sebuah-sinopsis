...
@section('content')
<form id="book-form" method="POST" action="{{ route('admin.books.update', $book) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="max-w-6xl mx-auto flex flex-col gap-6">
        ...
        <div class="flex flex-col gap-1">
            <h2 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Edit Post</h2>
            <p class="text-gray-500 dark:text-gray-400 text-sm">Update the details of "{{ $book->title }}"</p>
        </div>
            ...
            <div class="lg:col-span-2 flex flex-col gap-6">
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <label class="flex flex-col">
                                ...
                                <input type="text" name="author" value="{{ old('author', $book->author) }}" class="form-input w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white h-11 px-4 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors text-sm" placeholder="Author Name" required/>
                            </label>
                            <label class="flex flex-col">
                                <p class="text-gray-700 dark:text-gray-300 text-sm font-medium mb-1.5">Publisher</p>
                                <input type="text" name="publisher" value="{{ old('publisher', $book->publisher) }}" class="form-input w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white h-11 px-4 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors text-sm" placeholder="Publisher Name"/>
                            </label>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <label class="flex flex-col">
                                ...
                                <input type="text" name="isbn" value="{{ old('isbn', $book->isbn ?? '') }}" class="form-input w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white h-11 px-4 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors text-sm" placeholder="e.g. 978-3-16-148410-0"/>
                            </label>
                            <label class="flex flex-col">
                ...
                <div class="bg-surface-light dark:bg-surface-dark rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-6 flex flex-col flex-1">
                    ...
                    <div class="flex flex-col rounded-lg border border-gray-300 dark:border-gray-700 overflow-hidden focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all flex-1">
                        ...
                        <!-- Editor Area -->
                        <textarea name="synopsis" class="w-full min-h-[280px] p-4 bg-white dark:bg-gray-900 text-gray-900 dark:text-white border-none focus:ring-0 resize-y text-sm leading-relaxed" placeholder="Write the book synopsis and review here..." required>{{ old('synopsis', $book->synopsis) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-6">
                ...
                <div class="bg-surface-light dark:bg-surface-dark rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-6">
                    ...
                    <label for="cover_image" class="block">
                        <div id="cover-preview" class="w-full aspect-[3/4] border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg flex flex-col items-center justify-center p-6 text-center hover:border-primary hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-all cursor-pointer group overflow-hidden">
                            @if ($book->cover_image)
                    ...
                    @if ($book->cover_image)
                        <p class="text-xs text-gray-500 mt-2 text-center">Click to replace cover image</p>
                    @endif
                </div>
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <div id="affiliate-container" class="flex flex-col gap-2">
                            @if ($book->affiliate_links)
                                @foreach ($book->affiliate_links as $index => $link)
                                    @if (!empty($link['name']) && !empty($link['url']))
                                        <div class="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700">
                                            <input type="hidden" name="affiliate_links[{{ $index }}][name]" value="{{ $link['name'] }}">
                                            <input type="hidden" name="affiliate_links[{{ $index }}][url]" value="{{ $link['url'] }}">
                                            <div class="flex items-center gap-3">
                                                <div class="w-9 h-9 rounded-lg bg-white dark:bg-gray-700 flex items-center justify-center shadow-sm border border-gray-100 dark:border-gray-600">
                                                    <span class="text-[10px] font-bold text-primary">{{ strtoupper(substr($link['name'], 0, 3)) }}</span>
                                                </div>
                                                <div class="flex flex-col">
                                                    <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $link['name'] }}</span>
                                                    <span class="text-xs text-gray-500 truncate max-w-[140px]">{{ $link['url'] }}</span>
                                                </div>
                                            </div>
                ...
                <div class="bg-surface-light dark:bg-surface-dark rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-6">
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <label class="flex flex-col">
                            ...
                            <div class="relative">
                                <select name="is_published" class="form-select w-full rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white h-10 px-3 pr-10 focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors appearance-none text-sm">
                                    <option value="1" {{ old('is_published', $book->is_published ?? 1) == 1 ? 'selected' : '' }}>Published</option>
                                    <option value="0" {{ old('is_published', $book->is_published ?? 1) == 0 ? 'selected' : '' }}>Draft</option>
                                </select>
                                <span class="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-gray-400">
                        ...
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $book->is_featured ?? 0) ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 dark:border-gray-600 text-primary focus:ring-primary/20">
                            <div>
                        ...
                        <!-- Allow Comments -->
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="allow_comments" value="1" {{ old('allow_comments', $book->allow_comments ?? 1) ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 dark:border-gray-600 text-primary focus:ring-primary/20">
                            <div>
...
@push('scripts')
<script>
    let affiliateLinkIndex = {{ $book->affiliate_links ? count($book->affiliate_links) : 0 }};
